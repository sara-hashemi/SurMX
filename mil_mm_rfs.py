...
Original file is located at
    https://colab.research.google.com/drive/1PG3h5ZwJBOEnFH05pqX9JwtVkjdt6K1u
"""
...
#   CONFIGURATION
# =====================================================
TARGET_EVENT = "recurrence"

device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"Using device: {device}")
print(f"Training for Target: {TARGET_EVENT.upper()}")

EMB_DIM = 16
NUM_NUMERIC = 4
BATCH_SIZE = 16 # Increased batch size slightly for stability
LR = 2e-4
EPOCHS = 20
...
# =============================================================
#       MANUAL METRIC (Fix for datetime.UTC error)
# =============================================================
...
# =============================================================
#       BUILD CPU-STORED TENSORS
# =============================================================
bags = {}
globals_dict = {}
clinical_dict = {}
targets = {} # Will hold (time, event) for the chosen target
...
for pid, g in df.groupby("patient_id"):
    ...
    # Select the specific target we care about
...
print(f"✔ All patient tensors built. Target: {TARGET_EVENT}")
...
class ClinicalEncoder(nn.Module):
    def __init__(self, cat_dims):
        ...
        # Slightly wider layer for fused features
...
class SingleTaskSurvModel(nn.Module):
    def __init__(self, cat_dims):
        ...
        self.enc = ResMLP()
        self.mil = MultiHeadAttnMIL()
        self.glb = nn.Sequential(nn.Linear(256, 256), nn.ReLU())

        # Fusing: MIL(256) + Global(256) + Clinical(128) = 640
...
def cox_loss(risk, time, event):
    ...
    # Only calculate loss for uncensored events
...
#       TRAINING UTILS
# =============================================================
def get_batch_data(pids, model):
    # Helper to gather data for a list of pids
    # returns list of inputs ready for batch processing if we were parallel
    # Since inputs are variable length (bags), we loop inside the train step
    pass
...
def evaluate(model, test_ids):
    ...
    # Calculate C-Index
    # Note: concordance_index expects predictions where higher = higher risk.
    # Cox output is log-hazard. Higher log-hazard = lower survival time.
    # Lifelines expects "predicted scores", where higher score = shorter life = higher risk.
    # So we pass raw risk directly.
    try:
        c_index = concordance_index(times, risks, events)
    except:
        c_index = 0.5 # Handle edge case of no events in test set
...
def train_one_fold(train_ids, test_ids):
    ...
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-4) # AdamW is better for generalization
    ...
    for ep in range(EPOCHS):
        ...
        for i in range(0, len(train_ids), BATCH_SIZE):
            ...
            # Accumulate predictions for batch loss calculation
            # Note: Cox Loss requires seeing the whole batch at once to compare risks
            batch_risks = []
            batch_times = []
            batch_events = []
            ...
            if len(batch_events) < 2: continue # Skip if batch too small (tail end)
            if sum(batch_events) < 1: continue # Skip if no events in batch (grad is zero)
...
# Split 80/20 ensuring equal event ratio in train/test
...
# Save
torch.save(best_wts, f"miccai_model_{TARGET_EVENT}.pth")
print(f"\nSaved best {TARGET_EVENT} model with C-Index: {final_score:.4f}")